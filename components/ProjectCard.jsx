import Link from "next/link";
import Image from "next/image";
import { Card, CardHeader } from "./ui/card";
import { RiGithubFill } from "react-icons/ri";
import { Link2Icon } from "lucide-react";
import { Badge } from "./ui/badge";
const ProjectCard = ({ project }) => {
  return (
    <Card className="group overflow-hidden relative">
      <CardHeader className="p-0">
        <div
          className="relative w-full h-[300px] flex items-center justify-center bg-tertiary dark:bg-secondary/40 
        bg-work_project_bg_light 
        dark:bg-work_project_bg_dark
        bg-no-repeat overflow-hidden
        "
        >
          <Image
            className="absolute  top-2 bottom-0 shadow-2xl"
            src={project?.image}
            width={200}
            height={280}
            alt="project-banner"
            priority
          />
          <div>
            <Link
              className="bg-secondary w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 group-hover:opacity-100
              group-hover:scale-100
               transition-all duration-200 border border-[#FE6868]"
              href={project.link}
            >
              <Link2Icon className="text-[#FE6868]" />
            </Link>
            <Link
              className="bg-secondary w-[54px] h-[54px] rounded-full flex justify-center items-center scale-0 opacity-0 
              group-hover:scale-100 group-hover:opacity-100 transition-all duration-200
              border border-[#FE6868]
              "
              href={project?.github}
            >
              <RiGithubFill size={28} className="text-[#FE6868] " />
            </Link>
          </div>
        </div>
      </CardHeader>
      <div className="h-full px-8 py-6">
        <Badge
          className={"uppercase text-sm font-medium mb-2 absolute top-4 left-5"}
        >
          {project.category}
        </Badge>
        <h4 className="h4 mb-1">{project.name}</h4>
        <p className="text-muted-foreground text-lg">{project.description}</p>
      </div>
    </Card>
  );
};

export default ProjectCard;
